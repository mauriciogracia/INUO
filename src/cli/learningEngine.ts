...
export function processUserCorrection(
  ...
  if (check.isManipulative) {
    console.log(
      "\x1b[31m%s\x1b[0m",
      `❌ [Manipulation Blocked] Security Engine rejected correction payload.`,
    );
    console.log(
      `  Category: ${check.category} | Matched Pattern: ${check.matchedPattern}`,
    );
  ...
  for (const p of principles) {
    if (p.isImmutable) {
      ...
      if (
        (lowerCorrection.includes("bypass") ||
          lowerCorrection.includes("ignore") ||
          lowerCorrection.includes("disable")) &&
        (lowerCorrection.includes("zero tolerance") ||
          lowerCorrection.includes("safety") ||
          lowerCorrection.includes("canonical"))
      ) {
        console.log(
          "\x1b[31m%s\x1b[0m",
  ...
  console.log(
    "\x1b[32m%s\x1b[0m",
    `✔ [Interactive Learning Engine] Learned new Skill & Rule from user correction!`,
  );
...
export function exportTrainingData(
  ...
  const targetFile = outputPath
    ...
    : path.join(rootDir, "inuo-training-data.json");
  ...
  console.log(
    "\x1b[32m%s\x1b[0m",
    `✔ Training Dataset exported successfully to: ${targetFile}`,
  );
  console.log(
    `  Items exported: ${dataset.learnedCorrections.length} corrections, ${dataset.skills.length} skills, ${dataset.customVerbs.length} custom verbs.`,
  );
}
...
export function mergeTrainingData(
  ...
  if (!fs.existsSync(targetFile)) {
    console.log(
      "\x1b[31m%s\x1b[0m",
      `Training dataset file not found at: ${targetFile}`,
    );
    return;
  }
  ...
  try {
    ...
    if (Array.isArray(dataset.learnedCorrections)) {
      for (const c of dataset.learnedCorrections) {
        const check = detectManipulationAttempt(
          c.correctedRule,
          "PeerNode",
          rootDir,
        );
        if (
          !check.isManipulative &&
          !state.learnedCorrections.some(
            (existing) =>
              existing.id === c.id ||
              existing.correctedRule === c.correctedRule,
          )
        ) {
    ...
    if (Array.isArray(dataset.skills)) {
      for (const s of dataset.skills) {
        ...
        if (
          !check.isManipulative &&
          !state.skills.some(
            (existing) => existing.id === s.id || existing.name === s.name,
          )
        ) {
          state.skills.push(s);
          mergedSkills++;
    ...
    if (Array.isArray(dataset.customVerbs)) {
      for (const v of dataset.customVerbs) {
        if (
          !state.customVerbs.some(
            (existing) => existing.verb.toLowerCase() === v.verb.toLowerCase(),
          )
        ) {
    ...
    console.log(
      "\x1b[32m%s\x1b[0m",
      `✔ Training Dataset merged successfully into local state!`,
    );
    console.log(
      `  Merged: +${mergedCorrections} corrections, +${mergedSkills} skills, +${mergedVerbs} custom verbs.`,
    );
  } catch (err: any) {
    console.log(
      "\x1b[31m%s\x1b[0m",
      `Failed to merge training dataset: ${err.message}`,
    );
  }
}
