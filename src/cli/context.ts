...
export const BASELINE_ENGINES: EngineConfig[] = [
  {
    ...
    description: 'Collection of behaviors governing trust scoring, prompt injection defense, and sub-2ms circuit breaker disconnects.',
    behaviorIds: ['behavior_anti_manipulation', 'behavior_circuit_breaker', 'behavior_trusted_members'],
  ...
  },
  {
    ...
    description: 'Collection of behaviors handling owner incapacitation, family fallback authorization, and stranger command defense.',
    behaviorIds: ['behavior_owner_incapacitation', 'behavior_family_emergency', 'behavior_stranger_defense'],
    createdBy: 'MasterTrainer',
    isImmutable: true,
    updatedAt: '2026-08-14T00:00:00.000Z',
  },
  {
    ...
    description: 'Collection of behaviors governing platform self-reflection, versioning, and trust-gated spec disclosures.',
    behaviorIds: ['behavior_trust_gated_self_reflection', 'behavior_spec_disclosure'],
    createdBy: 'MasterTrainer',
    isImmutable: true,
    updatedAt: '2026-08-14T00:00:00.000Z',
  },
...
export const BASELINE_PRINCIPLES: Principle[] = [
  {
    ...
    statement: 'Strict enforcement of zero-tolerance policies prohibiting illegal exploitation and harmful transactions.',
    createdBy: 'MasterTrainer',
    isImmutable: true,
    status: 'Locked',
  ...
  },
  {
    ...
    statement: 'Every interaction unit MUST adhere to NEED = (VERB) + (OBJECT) and OFFER = (COMP_VERB) + (OBJECT).',
    createdBy: 'MasterTrainer',
    isImmutable: true,
    status: 'Locked',
    createdAt: new Date().toISOString(),
...
export function loadState(statePath: string): StateData {
  ...
  try {
    ...
    return {
      ...
      skills: parsed.skills && parsed.skills.length > 0 ? parsed.skills : [...BASELINE_SKILLS],
      behaviors: parsed.behaviors && parsed.behaviors.length > 0 ? parsed.behaviors : [...BASELINE_BEHAVIORS],
      rules: parsed.rules || [],
      principles: parsed.principles && parsed.principles.length > 0 ? parsed.principles : [...BASELINE_PRINCIPLES],
