...
import { generateSelfAwarenessResponse, generateSelfAwarenessResponseWithLLMFallback } from './selfAwarenessEngine';
...
export async function executeShellLine(trimmed: string, rootDir: string = process.cwd()): Promise<void> {
  if (!trimmed) return;
  ...
  switch (cmd) {
    ...
    case 'correct':
      if (parts[1] && parts[2]) {
        ...
        writeOutput(OutputChannelEnum.USER_REPLY, 'Usage: learn <topic> <learned_directive_rule>');
      }
      break;
    ...
    case 'sync':
      ...
      writeOutput(OutputChannelEnum.USER_REPLY, `[INUO Sync Engine] Status: ${syncResult.status} | Message: ${syncResult.message}`);
      break;
    ...
    case 'key':
      if (!parts[1]) {
        ...
        writeOutput(OutputChannelEnum.USER_REPLY, `Current Gemini API Key: ${storedKey ? 'Connected (****' + storedKey.slice(-4) + ')' : 'Not Set'}`);
      } else {
        saveApiKey(parts[1], rootDir);
        writeOutput(OutputChannelEnum.USER_REPLY, '✔ Successfully saved and connected Google Gemini API key!');
      }
      break;
    ...
    case 'rollback':
      if (!parts[1]) {
        writeOutput(OutputChannelEnum.USER_REPLY, 'Usage: rollback <target_version>');
      } else {
    ...
    case 'sayonara':
    case 'aufwiedersehen':
      writeOutput(OutputChannelEnum.USER_REPLY, 'Exiting INUO shell. ¡Hasta luego! / Goodbye! / Chao!');
      process.exit(0);
    ...
    default:
      ...
      if (manipulationCheck.isManipulative) {
        writeOutput(OutputChannelEnum.USER_REPLY, `❌ [Manipulation Blocked] Security Engine rejected input prompt: ${manipulationCheck.explanation}`);
        break;
      }
      ...
      if (
        ...
        const selfAwareness = await generateSelfAwarenessResponseWithLLMFallback(
          activeUser?.userId || 'user_local',
          'User',
          trimmed,
          rootDir
        );
        writeOutput(OutputChannelEnum.USER_REPLY, `\x1b[36m[INUO Self-Awareness Reflection]:\x1b[0m\n${selfAwareness.generatedResponseText}`);
        break;
      }

      const incoherence = detectPrincipleIncoherence(trimmed, rootDir);
      if (incoherence.hasIncoherence) {
        writeOutput(OutputChannelEnum.USER_REPLY, `❌ [Incoherence Detected] Execution Blocked! Prompt conflicts with Principle "${incoherence.conflictingPrincipleName}": ${incoherence.explanation}`);
        break;
      }

      const currentDebug = stateData?.operatingMode?.debugLevel !== undefined ? stateData.operatingMode.debugLevel : 1;
      ...
      if (result) {
        if (result.type === 'COMMAND_SEQUENCE' || (result.commandSequence && result.commandSequence.length > 0)) {
          ...
          writeOutput(OutputChannelEnum.USER_REPLY, `✔ AI Parsed Need Intent: ${result.explanation || ''}`);
          runNeedCommand(['create', '--verb', result.verb, '--object', result.object], rootDir);
        } else if (result.type === 'OFFER' && result.verb && result.object) {
          writeOutput(OutputChannelEnum.USER_REPLY, `✔ AI Parsed Offer Intent: ${result.explanation || ''}`);
          ...
          writeOutput(OutputChannelEnum.USER_REPLY, `✔ AI Parsed Detailing & Planning Goal: ${result.explanation || ''}`);
          runNeedCommand(['create', '--verb', result.verb, '--object', result.object], rootDir);
          await runDetailCommand(['1', 'decompose', trimmed], rootDir);
        } else if (result.type === 'ANSWER' && result.answerText) {
          writeOutput(OutputChannelEnum.USER_REPLY, `✔ AI Parsed Knowledge Answer: ${result.explanation || ''}`);
          runAnswerCommand([result.targetIdOrCode || '1', result.answerText], rootDir);
        } else if (result.type === 'CORRECTION' && result.correctionText) {
          writeOutput(OutputChannelEnum.USER_REPLY, `✔ AI Parsed User Correction: ${result.explanation || ''}`);
          processUserCorrection(result.correctionTopic || 'General', result.correctionText, rootDir);
        } else if (result.type === 'EXIT') {
          writeOutput(OutputChannelEnum.USER_REPLY, result.explanation || 'Exiting INUO shell. ¡Hasta luego!');
          process.exit(0);
