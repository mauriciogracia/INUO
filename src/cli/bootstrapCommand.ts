...
export function runBootstrap(rootDir: string = process.cwd()): void {
  ...
  console.log('\x1b[36m%s\x1b[0m', '=== INUO Seed Agent Bootstrap Protocol ===');
...
`;
    ...
    console.log('\x1b[32m%s\x1b[0m', '✔ Validated existing system prompt: INUO_SPEC.md');
  }
  ...
  if (!fs.existsSync(paths.manifestPath)) {
    ...
    console.log('\x1b[32m%s\x1b[0m', '✔ Generated baseline manifest: inuo-manifest.json (v00.95.02)');
  } else {
    console.log('\x1b[32m%s\x1b[0m', '✔ Validated existing manifest: inuo-manifest.json');
  }

  console.log('\x1b[32m%s\x1b[0m', '✔ Single-definition directories verified: enums/, types/, interfaces/');
  console.log('\x1b[33m%s\x1b[0m', '★ Environment bootstrap sequence complete!');
}
