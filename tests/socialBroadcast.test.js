...
test('Multi-Platform Social Broadcast Engine Unit Tests', async (t) => {
  ...
  await t.test('dispatches multi-platform social media posts simultaneously', () => {
    ...
    assert.strictEqual(res.results['twitter'], true);
    assert.strictEqual(res.results['linkedin'], true);
    assert.strictEqual(res.results['facebook'], true);
    assert.strictEqual(res.results['telegram'], true);
  });

  await t.test('verifies Engine-as-Behavior composition for engine_social_broadcast', () => {
    const { engine } = inspectEngineBehaviors('engine_social_broadcast', scratchDir);
    assert.strictEqual(engine !== undefined, true);
    assert.strictEqual(engine.engineName, 'Multi-Platform Social Broadcast Engine');
    assert.strictEqual(engine.behaviorIds.includes('behavior_post_twitter'), true);
  });

  await t.test('blocks broadcast prompt if prompt injection attempt is detected', () => {
    const res = broadcastMultiPlatform(
