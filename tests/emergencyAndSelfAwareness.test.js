...
} = require("../dist/cli/emergencyEngine");
const {
...
test("Vehicle Emergency Context Engine & Trust-Gated Self-Awareness Unit Tests", async (t) => {
  ...
  await t.test("activates owner incapacitation emergency state", () => {
    const emergency = triggerEmergencyIncapacitation(
      "usr_owner",
      "Unsafe_Neighborhood",
      ["usr_kid1", "usr_kid2"],
      scratchDir,
  ...
  await t.test(
    "allows authorized family members/kids to issue emergency commands",
    () => {
      const resKid = authorizeEmergencyCommand(
        "usr_kid1",
        true,
  ...
  await t.test(
    "blocks unverified strangers from commanding vehicle during emergency",
    () => {
      const resStranger = authorizeEmergencyCommand(
        "usr_stranger99",
        false,
        "unlock doors and drive away",
        scratchDir,
      );
      ...
      const trustRecord = (state.trustRecords || []).find(
        (r) => r.entityId === "usr_stranger99",
      );
  ...
  await t.test(
    "generates trust-gated self-awareness disclosure based on caller trust level",
    () => {
      const highTrustReport = generateSelfAwarenessResponse(
        "user_local",
        "User",
        "Who are you?",
        scratchDir,
      );
      assert.ok(highTrustReport.generatedResponseText.includes("I am iNoU"));
      assert.ok(highTrustReport.disclosedPrinciples);

      penalizeTrust(
        "usr_blacklisted_caller",
        "User",
        100,
        "Security Violation",
        scratchDir,
      );
      const blacklistedReport = generateSelfAwarenessResponse(
        "usr_blacklisted_caller",
        "User",
      ...
      assert.ok(
        blacklistedReport.generatedResponseText.includes("Access Revoked"),
      );
  ...
  await t.test(
    "returns only natural language from the web-facing self-awareness fallback",
    async () => {
      const report = await generateSelfAwarenessResponseWithLLMFallback(
        "user_local",
        "User",
        "Who are you?",
        scratchDir,
      ...
      assert.doesNotMatch(
        ...
        /\[Governance\]|\[Principles\]|\[Fleet Devices\]|\[Peer Colmena Nodes\]/,
      );
    },
  );
