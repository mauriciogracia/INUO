...
const { detectLanguage, getLocalizedHostGreeting } = require('../dist/cli/languageEngine');
...
test('Multi-Lingual Intent & Language Detection Unit Tests (4 Languages)', async (t) => {
  ...
  await t.test('detects Spanish language for "what can you do" and "who are you" intents', () => {
  ...
  await t.test('detects French language for "what can you do" and "who are you" intents', () => {
  ...
  await t.test('detects German language for "what can you do" and "who are you" intents', () => {
  ...
  await t.test('detects Portuguese language for "what can you do" and "who are you" intents', () => {
  ...
  await t.test('provides localized host greetings across all 4 languages', () => {
    const esHost = getLocalizedHostGreeting('letMeServeYou', 'es', 'Sofia');
    assert.match(esHost.greetingText, /¡Buenos días Sofia!/);
    assert.match(esHost.promptWhoAreYouText, /¿Me permite saber quién nos acompaña/);

    const frHost = getLocalizedHostGreeting('letMeServeYou', 'fr', 'Jean');
    assert.match(frHost.greetingText, /Bonjour Jean/);
    assert.match(frHost.promptWhoAreYouText, /Puis-je savoir qui nous rejoint/);
  ...
  await t.test('generates self-awareness report describing system capabilities and formula', () => {
    const report = generateSelfAwarenessResponse('user_local', 'User', 'Who are you?', tmpDir);
    assert.match(report.generatedResponseText, /I am INUO/);
    assert.match(report.generatedResponseText, /NEED = \(VERB\) \+ \(OBJECT\)/);
  });
