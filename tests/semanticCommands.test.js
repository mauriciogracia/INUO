...
test("INOU Semantic Entity-Action Command Grammar Unit Tests", async (t) => {
  ...
  await t.test(
    "Entity token normalization resolves canonical and multilingual names",
    () => {
  ...
  await t.test(
    "Action token normalization resolves canonical and multilingual verbs",
    () => {
  ...
  await t.test(
    "Parser correctly extracts Entity, Action, Flags, and positional ID",
    () => {
  ...
  await t.test(
    "Project entity supports full CRUD lifecycle (add, update, enable, disable, remove, list)",
    async () => {
      await executeShellLine(
        "project add --name Alpha --jurisdiction US-CA",
        tmpDir,
      );
      ...
      await executeShellLine(
        `project update ${proj.id} --name Alpha-v2`,
        tmpDir,
      );
  ...
  await t.test("Workspace entity supports full CRUD lifecycle", async () => {
    ...
    await executeShellLine(
      `workspace add --path "${wsTarget}" --name SampleWorkspace`,
      tmpDir,
    );
    let state = loadState(path.join(tmpDir, ".inuo-state.json"));
  ...
  await t.test(
    "Task entity routes node, workflow, need and offer actions",
    async () => {
      // Add workflow
      await executeShellLine(
        "task add --type workflow --name CorePipeline",
        tmpDir,
      );
      // Add node
      await executeShellLine(
        "task add --title BuildModule --role Developer",
        tmpDir,
      );
      // Add need & offer
      await executeShellLine(
        "task add --type need --verb Request --object Storage",
        tmpDir,
      );
      await executeShellLine(
        "task add --type offer --verb Provide --object Storage",
        tmpDir,
      );
  ...
  await t.test(
    "Preference entity manages UI mode, roles, aliases, and settings",
    async () => {
      await executeShellLine(
        "preference update --key mode --value letMeServeYou",
        tmpDir,
      );
      let state = loadState(path.join(tmpDir, ".inuo-state.json"));
      ...
      await executeShellLine(
        "preference update --key role --value MasterTrainer",
        tmpDir,
      );
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
      ...
      await executeShellLine(
        "preference add --key alias --name quick-status --target status",
        tmpDir,
      );
      await executeShellLine(
        "preference update --key auto_sync_interval --value 20",
        tmpDir,
      );
      await executeShellLine(
        "preference add --key llm_provider --value gemini-pro --project ProjectA",
        tmpDir,
      );
      await executeShellLine(
        "preference add --key theme --value dark --workspace WorkspaceB",
        tmpDir,
      );
      await executeShellLine(
        "preference add --key timeout --value 30 --task TaskC",
        tmpDir,
      );
      ...
      assert.equal(
        state.preferences["project:ProjectA:llm_provider"].value,
        "gemini-pro",
      );
      assert.equal(
        state.preferences["workspace:WorkspaceB:theme"].value,
        "dark",
      );
      assert.equal(state.preferences["task:TaskC:timeout"].value, "30");

      await executeShellLine(
        "preference disable --key theme --workspace WorkspaceB",
        tmpDir,
      );
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
      assert.equal(
        state.preferences["workspace:WorkspaceB:theme"].enabled,
        false,
      );

      await executeShellLine("preference list", tmpDir);
      await executeShellLine(
        "preference remove --key alias --name quick-status",
        tmpDir,
      );
    },
  );
  ...
  await t.test(
    "Chat entity supports full CRUD lifecycle (add, update, enable/disable, remove, list)",
    async () => {
      // Create a chat
      await executeShellLine("chat add --title MyConversation", tmpDir);
      let state = loadState(path.join(tmpDir, ".inuo-state.json"));
      assert.ok(state.chats && state.chats.length >= 1);
      const chat = state.chats[0];
      assert.equal(chat.title, "MyConversation");
      assert.equal(chat.status, "Active");
      assert.equal(state.activeChat, chat.id);

      // Update chat
      await executeShellLine(
        `chat update ${chat.id} --title UpdatedConversation`,
        tmpDir,
      );
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
      assert.equal(state.chats[0].title, "UpdatedConversation");

      // Create another chat
      await executeShellLine("chat add --title SecondConversation", tmpDir);
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
      assert.equal(state.chats.length, 2);
      assert.equal(state.activeChat, state.chats[1].id);

      // Disable/Archive first chat
      await executeShellLine(`chat disable ${chat.id}`, tmpDir);
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
      assert.equal(state.chats[0].status, "Archived");

      // Activate first chat again
      await executeShellLine(`chat enable ${chat.id}`, tmpDir);
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
      assert.equal(state.chats[0].status, "Active");

      // List all chats
      await executeShellLine("chat list", tmpDir);

      // Remove a chat
      await executeShellLine(`chat remove ${chat.id}`, tmpDir);
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
  ...
  await t.test(
    "Chat entity normalizes multilingual aliases (conversacion, conversation, gespräch)",
    () => {
      assert.equal(normalizeSemanticEntity("chat"), "chat");
      assert.equal(normalizeSemanticEntity("conversacion"), "chat");
      assert.equal(normalizeSemanticEntity("conversation"), "chat");
      assert.equal(normalizeSemanticEntity("gespräch"), "chat");
      assert.equal(normalizeSemanticEntity("ch"), "chat");
      assert.equal(normalizeSemanticEntity("c"), "chat");
    },
  );

  await t.test(
    "Multilingual Spanish commands execute transparently",
    async () => {
      ...
      await executeShellLine(
        "preferencia actualizar --key mode --value promptMe",
        tmpDir,
      );
      state = loadState(path.join(tmpDir, ".inuo-state.json"));
