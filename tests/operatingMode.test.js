...
const { setOperatingMode, setInteractionLanguage, initiateHostGreeting } = require('../dist/cli/hostServiceEngine');
...
test('Dual Operating Modes & Dynamic Language Determination Unit Tests', async (t) => {
  ...
  await t.test('switches operating mode to letMeServeYou host concierge mode', () => {
  ...
  await t.test('detects input language automatically across multi-lingual cues', () => {
    assert.strictEqual(detectLanguage('Hola buenos dias necesito ayuda'), 'es');
    ...
    assert.strictEqual(detectLanguage('Hello create a new need for water'), 'en');
  });

  await t.test('generates localized host concierge greetings in Spanish, French, and English', () => {
    setInteractionLanguage('es', true, scratchDir);

    const hostEs = initiateHostGreeting(scratchDir);
    assert.match(hostEs.greeting, /¡Buenos días/);
    assert.match(hostEs.promptMessage, /quién nos acompaña hoy/);

    const greetingFr = getLocalizedHostGreeting('letMeServeYou', 'fr', 'Sofia');
    assert.match(greetingFr.greetingText, /Bonjour Sofia/);
    assert.match(greetingFr.promptWhoAreYouText, /Puis-je savoir qui/);
  });
