...
test('Sync Engine Automated Detection & Verification Unit Tests', async (t) => {
  ...
  await t.test('detects when manifest and spec are synchronized', () => {
    ...
    assert.equal(result.currentManifestVersion, '0.1.0');
  });

  await t.test('detects spec version upgrade (e.g. SPEC_VERSION: "0.2.0") and applies verification update', () => {
    // Simulate updating SPEC_VERSION in INUO_SPEC.md to 0.2.0
...
`;
    ...
    assert.equal(result.currentManifestVersion, '0.2.0');

    // Verify manifest updated
    const manifest = loadManifest(manifestPath);
    assert.equal(manifest.SPEC_VERSION, '0.2.0');
  });
