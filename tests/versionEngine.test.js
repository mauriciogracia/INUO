...
test('INUO Canonical Versioning (Deployed.SpecRevision.Implementation) Unit Tests', async (t) => {
  ...
  await t.test('formats numbers into zero-padded Deployed.SpecRevision.Implementation version strings', () => {
  ...
  await t.test('calculates system version matching target 00.02.95', () => {
    const sysVer = calculateInuoVersion(scratchDir);
    assert.strictEqual(sysVer.fullVersionString, '00.02.95');
    assert.strictEqual(sysVer.deployedPercentage, 0);
    assert.strictEqual(specRevisionIndex = sysVer.specRevisionIndex, 2);
  ...
  await t.test('recalculates and synchronizes version across package.json, manifest, and spec from single source', () => {
    ...
    assert.strictEqual(res.fullVersionString, '00.02.95');

    const updatedPkg = JSON.parse(fs.readFileSync(dummyPkg, 'utf8'));
    assert.strictEqual(updatedPkg.version, '00.02.95');

    const updatedManifest = JSON.parse(fs.readFileSync(dummyManifest, 'utf8'));
    assert.strictEqual(updatedManifest.SPEC_VERSION, '00.02.95');

    const updatedSpec = fs.readFileSync(dummySpec, 'utf8');
    assert.match(updatedSpec, /"00\.02\.95"/);
  });
